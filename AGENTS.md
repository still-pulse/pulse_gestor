...
- Todos os identificadores técnicos de DocTypes, Reports e rotas devem usar apenas ASCII, sem acentos, cedilha ou outros caracteres especiais. Isso inclui `name`/`report_name`, chaves `frappe.query_reports`, `link_to` no Workspace e nomes de diretórios. Rótulos visíveis ao usuário podem ter acentos. Conferir essa regra antes de migrar novos metadados.
