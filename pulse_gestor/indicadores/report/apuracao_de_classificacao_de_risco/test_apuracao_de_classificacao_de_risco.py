...
class TestApuracaoDeClassificacaoDeRisco(FrappeTestCase):
	...
	def test_periodo_com_duas_escalas_e_total_do_card(self):
		...
		protocolos = []
		for inicio, fim, alvo in (("2098-01-01", "2098-01-15", 30), ("2098-01-16", "2098-01-31", 60)):
			protocolo = frappe.get_doc(
				{
					"doctype": "Protocolo de Triagem",
					"nome_protocolo": f"Teste apuração {uuid4().hex[:10]}",
					"niveis": [
						{"ordem": 1, "nome_nivel": "Vermelho", "tempo_maximo_espera_min": 0},
						{"ordem": 2, "nome_nivel": "Amarelo", "tempo_maximo_espera_min": alvo},
					],
				}
			).insert()
			frappe.get_doc(
		...
		def lancar(data, vermelho, amarelo, enviar=True):
			doc = frappe.get_doc(
				{"doctype": "Classificacao de Risco Diaria", "unidade": unidade, "data": data}
			).insert()
		...
		columns, rows, _, _, summary = execute(
		...
		_, sem_pacientes, _, _, summary = execute(
